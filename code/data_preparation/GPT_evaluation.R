...
# data <- read_delim(paste0("data/speeches/speeches_", country, "_translated.csv"))
data <-
  read_delim(paste0("data/speeches/speeches_", country, ".csv"))
...
prompts <-
  tibble(
    prompts = c(
      "Evaluate the sentiment and its strength in the the text. Output only a number from -5 (very negative) to 5 (very positive) with 0.1 increments. Text:'",
      ...
      "Detect the emotion of disgust in the following text. Evaluate strength of the emotion on a scale. Output only a number from 0 (no disgust) to 5 (very disgusted) with 0.1 increments. Text: '"
    )
    ,
    emotions = c(
      "sentiment",
...
prompt_chat <-
  c(
    "Evaluate the sentiment and its strength in the the text. Output only a number from -5 (very negative) to 5 (very positive) with 0.1 increments. Do not provide any explanation.",
    ...
    "Detect the emotion of disgust in the following text. Evaluate strength of the emotion on a scale. Output only a number from 0 (no disgust) to 5 (very disgusted) with 0.1 increments. Do not provide any explanation."
  )

res <- matrix(NA, nrow = nrow(df), ncol = length(prompt_chat))
# res <- cbind(read.csv(paste0("/Users/gustawkempa/Words-To-Votes/data/emotions/",country,"/", country,"_emotions.csv")),NA)
colnames(res) <-
  c(
    "sentiment",
    "happiness",
...
# as the API sometimes does not work we will iterate through the whole dataset 5 times
for (iter in 1:5) {
  # We will iterate through all of the prompts
  for (j in 1:length(prompt_chat)) {
    # And we will iterate through all of the fragments
...
write_csv(country_data,
          paste0("data/emotions/", country, "/", country, "_emotions.csv"))
