...
links <- c('https://web.archive.org/web/20201024034506/http://www.governo.it/it/articolo/coronavirus-dichiarazione-del-presidente-conte/14297',
           ...
           #Change of government - how do we approach that? - lets ignore these
...
data_IT$speeches_IT[26] <- "Ci troviamo tutti di fronte, in questi giorni, a un nuovo peggioramento dell’emergenza sanitaria. Ognuno deve fare la propria parte nel contenere la diffusione del virus. Ma soprattutto il governo deve fare la sua. Anzi deve cercare ogni giorno di fare di più.
...
write.csv(data, 'datasets/speeches/speeches_IT_translated.csv')
